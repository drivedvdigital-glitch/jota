...
class ProdutoExtraido(BaseModel):
    title: str = Field(description="Título curto e direto do produto.")
    seo_description: str = Field(description="Descrição otimizada para SEO e conversão.")
    price: str = Field(description="Preço numérico encontrado.")
    features: List[str] = Field(description="Lista com 3 a 5 benefícios principais do produto.")
    description_html: str = Field(description="HTML de la descripción original traducido al español de Colombia. Todos los textos deben ser traducidos, pero TODAS las etiquetas HTML (especialmente img y gif, sus atributos src, alt, class, style, etc.) deben permanecer exactamente iguales.")
...
async def scrape_page_content(url: str) -> Dict[str, Any]:
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # Extrai imagens prioritariamente da galeria principal/carrossel do produto
...
async def extract_product_data(url: str) -> Dict[str, Any]:
    ...
    Sua tarefa é analisar o texto bruto de uma página de vendas e seu HTML de descrição, extrair os dados cruciais e TRADUZIR TUDO para o Espanhol da Colômbia.

    REGRAS INEGOCIÁVEIS:
    1. O idioma de saída DEVE ser estritamente o Espanhol focado no consumidor da Colômbia. Use tom persuasivo e direto.
    ...
    4. A 'seo_description' DEVE ter no máximo 90 caracteres (já em espanhol).
    5. Limpe qualquer referência ao nome da loja original ou de concorrentes.
    6. No campo 'description_html', você receberá o HTML da descrição original do produto. Você deve traduzir todos os textos visíveis dentro desse HTML para o Espanhol da Colômbia, mantendo exatamente todas as tags HTML (especialmente <img> contendo imagens e gifs, inclusive com seus atributos src, style, width, height, etc.) intactas e no mesmo lugar. Não altere as URLs das imagens/gifs! Se o HTML original estiver vazio ou ausente, construa uma descrição rica em HTML com os benefícios (features) e a descrição (seo_description) formatados usando títulos, listas e parágrafos persuasivos em espanhol da Colômbia.
    ...
    try:
        ...
        # Converte o preço original em USD para COP (Câmbio 3900 + Margem 3x) com tratamento robusto
        ...
        price_numbers = re.sub(r'[^\d.,]', '', price_raw)
        if ',' in price_numbers and '.' in price_numbers:
            price_numbers = price_numbers.replace(',', '')
        elif ',' in price_numbers:
            price_numbers = price_numbers.replace(',', '.')
            
        try:
            preco_original_usd = float(price_numbers) if price_numbers else 0.0
        except (ValueError, TypeError):
            preco_original_usd = 0.0
            
        preco_final_cop = preco_original_usd * 3900 * 3
        dados_limpos["price"] = f"COP {preco_final_cop:,.2f}"
